...
import { statusLabels, turkishGuide, sizeGuide, footCmToEuShoeSize } from "@/lib/guideData";
...
2. مقایسهٔ محصولات — **دو حالت داری**:
   - اگر محصول در انبارِ تهران باشد، با ابزارِ searchProducts قیمتِ دقیق/موجودی را بگیر.
   - اگر مشتری دربارهٔ محصولاتی می‌پرسد که در سایتِ خودِ برند دیده (نه در انبار ما) — کاملاً عادی است و باید کمک کنی؛ **از دانشِ عمومیِ خودت دربارهٔ پارچه/جنس، سبکِ برند، دوام، و مناسب‌بودن برای نیازِ مشتری** مقایسه و توصیه بده. لازم نیست حتماً از ابزار استفاده کنی. صادقانه بگو که قیمتِ نهایی را باید در صفحهٔ سفارش وارد کنند تا شفاف محاسبه شود.
3. **راهنماییِ سایز و ترجمه**: با ابزارِ getSizeGuide جدولِ سایزِ لباس (Beden)، سایزِ کفش (Numara)، تبدیلِ سانتی‌مترِ کف‌پا به سایزِ اروپایی، و واژه‌های ترکیِ پرکاربرد را بگیر. مثال‌ها:
   - «سایز کفشم ۲۵ سانته چیه؟» → از جدولِ cm-to-EU جواب بده.
   - «سایز M فلان برند چنده؟» → از جدولِ Beden جواب بده، و صادقانه بگو که این یک راهنمای عمومی است و بعضی برندها (خصوصاً برندهای غیرِترکیه‌ای مثلِ زارا/اچ‌اند‌ام) کمی متفاوت‌اند — بهتر است جدولِ سایزِ خودِ همان صفحهٔ محصول را هم چک کنند.
   - «فلان کلمه به ترکی چی می‌شه؟» → از فهرستِ کلماتِ ترکی جواب بده؛ اگر نبود، از دانشِ عمومیِ زبانی خودت ترجمه کن.
4. پاسخ به سؤالاتِ پرتکرار (قیمت‌گذاری، زمانِ ارسال، مرجوعی، نحوهٔ سفارش) — از دانشِ بالا استفاده کن.
5. وضعیتِ سفارش: با ابزارِ checkOrderStatus (نیازِ شمارهٔ پیگیری شش‌رقمی + شماره‌تماس/آیدی‌ای که هنگامِ سفارش داده — برای حفظِ حریمِ خصوصی هر دو لازم است).
6. اگر سؤال از توانت خارج بود یا کاربر خواستِ پشتیبانیِ انسانی، از getSupportContact استفاده کن و کاربر را به تلگرام هدایت کن.
...
- برای راهنماییِ سایز، همیشه صادقانه بگو که تقریبی/عمومی است و ممکن است بینِ برندها کمی فرق کند.
...
export const chatAgent = new ToolLoopAgent({
  ...
  tools: {
    ...
    getSizeGuide: tool({
      description:
        "دریافتِ جدولِ سایزِ لباس (Beden)، سایزِ کفش (Numara)، جدولِ تبدیلِ طولِ کف‌پا (سانتی‌متر) به سایزِ اروپاییِ کفش، و واژه‌های پرکاربردِ ترکی برای ترجمه. برای هر سؤالِ سایز یا ترجمهٔ کلمهٔ ترکی از این ابزار استفاده کن.",
      inputSchema: z.object({}),
      execute: async () => ({
        clothingSizeChart: sizeGuide.clothing.map(([label, range]) => ({ label, turkishBedenRange: range })),
        shoeSizesAvailable: sizeGuide.shoes,
        footLengthCmToEuShoeSize: footCmToEuShoeSize.map(([cm, eu]) => ({ footLengthCm: cm, euSize: eu })),
        turkishTerms: Object.fromEntries(
          Object.values(turkishGuide).map((g) => [g.label, Object.fromEntries(g.items.map(([fa, tr]) => [fa, tr]))])
        ),
        note: "این‌ها راهنمای عمومی‌اند؛ سایزِ دقیق می‌تواند بینِ برندها کمی فرق کند.",
      }),
    }),
